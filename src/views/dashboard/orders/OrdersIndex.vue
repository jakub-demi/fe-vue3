...
import OrderStatusHistoryDialog from "@/components/pages/dashboard/orders/OrderStatusHistoryDialog.vue"

const ordersTable = ref<OrderDataTableT[]>([])
const loading = ref<boolean>(true)

const statusHistoryOrderId = ref<number>(-1)
const statusHistoryDialogShow = ref<boolean>(false)
...
const showOrderStatusHistory = (orderId: number) => {
  statusHistoryOrderId.value = orderId
  statusHistoryDialogShow.value = true
}
...
<template>
  <OrderStatusHistoryDialog
    v-model:order-id="statusHistoryOrderId"
    v-model:show="statusHistoryDialogShow"
  />
    ...
    >
      <template #body="slotProps: OrdersTableSlotPropsT">
        ...
        <span
          ...
          :class="
            slotProps.data.current_status.name === texts.orders.table.columns.currentStatus.noStatus
              ? 'italic'
              : ''
          "
        >
